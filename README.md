...
- 详情页左右两栏**各自独立滚动**：
  - **左栏＝题目描述**：所见即所得的 Markdown 实时编辑器，支持**上传 `.md`** 或**上传 PDF 题面**（内嵌查看），并保留一键跳转 LeetCode 原题。
  - **右栏＝解题笔记**：Markdown 实时编辑、上传 / 下载 `.md`。
...
- 笔记支持 **Markdown**（单界面实时编辑与排版）与 **PDF**（上传 / 内嵌查看）。
- 卡片悬停即可重命名 / 删除。
...
- **编辑**：Markdown 在同一界面中直接编辑并实时呈现排版，支持工具栏及加粗、斜体、链接、标题、列表等快捷键。
